...
public struct TheaterSeat: Hashable, Sendable {
    ...
    /// 14 m screen hangs at (0, 2.35, -10.2). The floor slopes toward the screen — front rows sit
    ...
    public var eyeOffset: SIMD3<Float> {
        switch (level, row) {
        case (.floor, .front): [0, -0.65, -3.77]
        case (.floor, .middle): .zero
        case (.floor, .back): [0, 0.45, 3.24]
        case (.balcony, .front): [0, 2.6, -4.32]
        case (.balcony, .middle): [0, 3.0, -0.82]
        case (.balcony, .back): [0, 3.4, 2.64]
        }
    }
