...
- **Add your own videos.** Pick one or several video files at once from Files; Cinema copies them into the app's own storage so they stay available across launches.
...
- **SharePlay.** Watch together with friends over FaceTime (inherited from the app's original sample foundation).
- **visionOS immersive environment.** Play video inside a custom Reality Composer Pro environment.

## Background

Cinema is built on top of Apple's "Destination Video" sample project. For background on the underlying architecture this was built from, see
